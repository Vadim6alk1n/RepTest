...
public class Game implements Runnable{
    ...
    private void press(MouseEvent e) {
        ...
        if (e.getButton()== MouseButton.SECONDARY) {
           ...
           targets.add(new SimpleGame.game.Target((float) targetx,(float)targety,yes));
        }
    ...
    public void update()
    {
        ...
        if (SimpleGame.game.Input.keyPressed[SimpleGame.game.Input.key_d]) {
            player.x+= player.speed;
        }
        if (SimpleGame.game.Input.keyPressed[4]==true ) {
            Input.keyPressed[4]=false;
            addTarget();
        ...
        for (var t: targets)
        {
            ...
            t.x+=t.speedx;
            t.y+=t.speedy;
            if (t.x >= screen.getWidth()) {
                t.speedx = -2 - (float)Math.random();
            }
            if (t.y >= screen.getHeight()) {
                t.speedy = -2 - (float)Math.random();
            }
            if (t.x < 0) {
                t.speedx = 2 + (float)Math.random() ;
            }
